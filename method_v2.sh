...
setup_vm_image() {
    ...
    # Test internet connectivity to the registry
    if ! curl -Is --connect-timeout 5 "https://${REGISTRY_URL%%/*}" >/dev/null; then
        print_status "WARN" "⚠️  Cannot reach ${REGISTRY_URL}. Internet might be restricted."
    fi
    ...
    case "${image,,}" in
        ubuntu*)
            image_variants=("${REGISTRY_URL}/library/ubuntu:24.04" "${REGISTRY_URL}/library/ubuntu:22.04" "${REGISTRY_URL}/library/ubuntu:20.04" "${REGISTRY_URL}/library/ubuntu:latest" "$image")
            ;;
        debian*)
            image_variants=("${REGISTRY_URL}/library/debian:bookworm" "${REGISTRY_URL}/library/debian:bullseye" "${REGISTRY_URL}/library/debian:latest" "$image")
            ;;
        alpine*)
            image_variants=("${REGISTRY_URL}/library/alpine:3.20" "${REGISTRY_URL}/library/alpine:3.19" "${REGISTRY_URL}/library/alpine:latest" "$image")
            ;;
        centos*)
    ...
    print_status "INFO"  "   1. No internet connection or proxy issues"
        print_status "INFO" "   2. Registry rate limit (${REGISTRY_URL})"
...
get_default_image() {
    local os_type="$1"
    case "${os_type,,}" in
        ubuntu*)  echo "${REGISTRY_URL}/library/ubuntu:22.04" ;;
        debian*)  echo "${REGISTRY_URL}/library/debian:bookworm" ;;
        alpine*)  echo "${REGISTRY_URL}/library/alpine:latest" ;;
        centos*)  echo "quay.io/centos/centos:stream9" ;;
        rocky*|almalinux*) echo "${REGISTRY_URL}/rockylinux/rockylinux:9-minimal" ;;
        fedora*)  echo "${REGISTRY_URL}/library/fedora:latest" ;;
        arch*)    echo "${REGISTRY_URL}/library/archlinux:latest" ;;
        *)        echo "${REGISTRY_URL}/library/ubuntu:22.04" ;;
    esac
}
