...
check_podman() {
    ...
    # Ensure registries and storage are configured (fix for CoCalc/Cloud environments)
...
EOF
    ...
    if [ ! -f "$HOME/.config/containers/storage.conf" ]; then
        print_status "INFO" "🔧 Configuring Podman storage driver (vfs)..."
...
setup_vm_image() {
    ...
    # Test internet connectivity to Docker Hub
    if ! curl -Is --connect-timeout 5 https://docker.io >/dev/null; then
        print_status "WARN" "⚠️  Cannot reach docker.io. Internet might be restricted."
    fi
